...
<div id="app">
    ...
    <div class="bottom-panel">
        <el-button :loading="upload" @click="updatePrice">Обновить цены</el-button>
...
export default {
    ...
    methods: {
        getData(){
            axios.get('http://test.menukhov.ru/newcalc/getdata.php').then(response => { this.rawData = (response.data) })
        },
        updatePrice() {
            this.upload=true;
            axios.get('http://test.menukhov.ru/newcalc/upload.php').then(response => {
                this.upload=false;
                if (response.data == 1) {
                    this.$message({
